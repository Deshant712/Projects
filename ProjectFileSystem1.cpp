...
#define REGULAR 1     //aapla ha project Regular type la support krnar ahe
#define SPECIAL 2    //special file mnje symbolic link, directory file, divice driver chi file
...
UFDT UFDTArr[MAXINODE];         // UFDT ha Structure Cha Array ahe mhnun tyala nav dil UFDTArr[5]
...
void man(char *name)
{
    ...
    else if(strcmp(name,"read") == 0)
    {
        ...
        printf("Usage : Read File_Name No_ Of_Bytes_To_Read\n");
    }
...
void DisplayHelp()
{
    printf("ls : ToList Out All The Files\n");
...
void CreateDILB()           
{
    int i = 1;
    PINODE newn = NULL;
    PINODE temp = head;     // head navacha kuthla hi pointer yala as a parameter mhnun alela nhi ala ahe, jo aplya kde yaycha PNOND head mhnun
                            // ata ithe ya function la ka nhi ala paramter there is a globle variable at line no. 58
                            // ha generally main chya aat mdhe asaycha tr yala baher kadhal as a global mhnun
    while(i<= MAXINODE)
    {
...
int LseekFile(int fd,int size,int from)
{
    ...
    if((UFDTArr[fd].ptrfiletable->mode == READ) || (UFDTArr[fd].ptrfiletable->mode == READ+WRITE))
    {
        ...
        else if(from == END)
        {
            ...
            if(((UFDTArr[fd].ptrfiletable->readoffset) + size) > 0) return -1;
            (UFDTArr[fd].ptrfiletable->readoffset) = (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size;   return -1;        
        }  
    }   
    else if(UFDTArr[fd].ptrfiletable->mode == WRITE)
    {
        ...
        else if(from == END)
        {
            if((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size > MAXFILESIZE)  return -1;
            if(((UFDTArr[fd].ptrfiletable->writeoffset) + size) < 0) return -1;
            if((UFDTArr[fd].ptrfiletable->writeoffset) = (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size);
        }
    }
}
...
int main()
{
    ...
    while(1)          // there is no condition(Unconditional loop), firat ch raha jo prynt koni break krt nhi to prynt
    {                  //yala Infinite loop ass mhntat khi lok pn ha Infinite loop nhi this is Unconditional Loop
        fflush(stdin);
        strcpy(str,"");

        printf("\nMarvellous VFS : > ");

        fgets(str,80,stdin);     // fgets is like a scanf function
        ...
        {
            if(strcmp(command[0],"create") == 0)  // command of 0 la compair kel with the create
            {
